...
libero_suites = [
    ...
    "libero_object",
    "libero_goal",
    "libero_goal_pretrain",
    "libero_goal_lifelong",
    "libero_90",
    "libero_10",
]
task_maps = {}
...
@register_benchmark
class LIBERO_GOAL_PRETRAIN(Benchmark):
    def __init__(self, task_order_index=0):
        super().__init__(task_order_index=task_order_index)
        assert (
                task_order_index == 0
        ), "[error] currently only support task order for 10-task suites"
        self.name = "libero_goal_pretrain"
        self._make_benchmark()


@register_benchmark
class LIBERO_GOAL_LIFELONG(Benchmark):
    def __init__(self, task_order_index=0):
        super().__init__(task_order_index=task_order_index)
        assert (
                task_order_index == 0
        ), "[error] currently only support task order for 10-task suites"
        self.name = "libero_goal_lifelong"
        self._make_benchmark()
