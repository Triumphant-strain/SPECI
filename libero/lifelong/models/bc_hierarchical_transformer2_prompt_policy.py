    ...
    def gram_schmidt(self, vv):
        ...
        for k in range(s, f):
            ...
            while redo:
                redo = False
                if s > 0:
                    vk = vv[:, k - pt].clone()  # 使用上一个任务的子技能库对应的技能向量进行初始化
                else:
    ...
    def forward(self, x_querry, l, x_block, train=False, task_id=None):
        ...
        if l in self.e_layers:
            ...
            if train:
                ...
                K = K[0:f]  # K torch.Size([10, emb_size])
                A = A[0:f]  # A torch.Size([10, emb_size])
                p = p[0:f]  # p torch.Size([10, 2, emb_size])

            # with attention and cosine sim
            # (b x 1 x d) * soft([1 x k x d]) = (b x k x d) -> attention = k x d
            batch_A = torch.stack([A.clone() for _ in range(x_querry.shape[0])], dim=0)
            a_querry = torch.einsum('Bbd,Bkd->Bbkd', x_querry, batch_A)
            ...
            P_ = torch.einsum('Bbk,Bkld->Bbld', aq_k, batch_p)  # aq_k即alpha，加权 (B, 50, 16, 64)
...
class BCHierarchicalTransformer2PromptPolicy(BasePolicy):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def __init__(self, cfg, shape_meta):
        ...
        self.temporal_transformer_action = TransformerDecoder_Action(
            input_size=embed_size,
            num_layers=policy_cfg.transformer_num_layers,
            num_heads=policy_cfg.transformer_num_heads,
            head_output_size=policy_cfg.transformer_head_output_size,
    ...
    def skill_inference_decode(self, x):
        pos_emb = self.temporal_position_encoding_fn(x)  # (T:10, E:64)
        x = x + pos_emb.unsqueeze(1)  # (B:32, T:10, num_modality:5, E:64)
        sh = x.shape
        self.temporal_transformer_skill.compute_mask(x.shape, [x.shape[0], x.shape[1] * x.shape[2] * 2, x.shape[3]])
        x = TensorUtils.join_dimensions(x, 1, 2)  # (B:32, T*num_modality:50, E:64)
    ...
    def action_execute_decode(self, x):
        # print("x: ", x.shape)  (1, 10, 64)
        pos_emb = self.temporal_position_encoding_fn(x)  # (10, 64)
        ...
        x = self.temporal_transformer_action(x)
        x = x.reshape(*sh)
        return x[:, :, 0]  # (B, T, E)
    ...
    def forward(self, data):
        x = self.spatial_encode(data)  # torch.Size([B:32, T:10, num_modalities:5, E:64])
        x, prompt_loss = self.skill_inference_decode(x)
        # print("after skill_inference_decode shape: ", x.shape)  (B:32, T:10, E:64)
