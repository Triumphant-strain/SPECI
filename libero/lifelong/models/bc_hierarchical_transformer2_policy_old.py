...
# TODO(xjk): 把上一时刻的动作也作为输入，需要修改dataset、config和网络
class BCHierarchicalTransformer2Policy(BasePolicy):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def __init__(self, cfg, shape_meta):
        ...
        self.temporal_transformer_skill = TransformerDecoder_Skill(
    ...
    def skill_inference_decode(self, x):
        ...
        memory = repeat(memory, "b n d -> (repeat b) n d", repeat=x.shape[0])  # (B:32, num_code:100, E:64)
        ...
        x = self.temporal_transformer_skill(x, memory)
        x = x.reshape(*sh)
        return x[:, :, 0]  # (B, T, E)
