...
class BCHierarchicalTransformerCPPromptPolicy(BasePolicy):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def skill_inference_decode(self, x):
        ...
        x, prompt_loss = self.temporal_transformer_skill(x, self.prompt, CP_U=self.CP_U, CP_V=self.CP_V, CP_C=self.CP_C)
        x = x.reshape(*sh)  # (B, T:10, num_modalities:5, E:384)
        # return x[:, :, 0], prompt_loss  # (B, T, E)  改了输出
