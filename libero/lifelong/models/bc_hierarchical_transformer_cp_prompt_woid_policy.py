...
class BCHierarchicalTransformerCPPromptWOIDPolicy(BasePolicy):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def cp_c_selection(self, data):
        ...
        # # 选最大的一个
        # CP_C_index = torch.argmax(cos_similarity, dim=0)
        # # print("CP_C_index", CP_C_index)
        # CP_C = CP_C_TMP[CP_C_index]
        #
        # cp_c_selection_max_loss = - cos_similarity[CP_C_index] * self.cp_c_selection_cof

        # 加权
        # CP_C = torch.einsum('k,kabc->abc', n_cos_similarity, CP_C_TMP)
        CP_C = self.CP_C
        cp_c_selection_max_loss = torch.zeros((1,), requires_grad=False).to('cuda')
