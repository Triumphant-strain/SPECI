...
class BasePolicy(nn.Module, metaclass=PolicyMeta):
    ...
    def cp_compute_loss(self, data, reduction="mean"):
    ...
    def prompt_compute_loss(self, data, reduction="mean"):
        data = self.preprocess_input(data, train_mode=True)
        dist, prompt_loss = self.forward(data)
        bc_loss = self.policy_head.loss_fn(dist, data["actions"], reduction)
        loss = bc_loss + prompt_loss
        return loss, prompt_loss

    def cp_prompt_woid_compute_loss(self, data, reduction="mean"):
        data = self.preprocess_input(data, train_mode=True)
        dist, prompt_loss, cp_c_loss = self.forward(data)
        bc_loss = self.policy_head.loss_fn(dist, data["actions"], reduction)
        loss = bc_loss + prompt_loss + cp_c_loss
        return loss, prompt_loss, cp_c_loss

    def cp_prompt_compute_loss(self, data, reduction="mean"):
        data = self.preprocess_input(data, train_mode=True)
        dist, prompt_loss = self.forward(data)
        bc_loss = self.policy_head.loss_fn(dist, data["actions"], reduction)
        loss = bc_loss + prompt_loss
