...
class CodaPrompt(nn.Module):
    def __init__(self, policy_cfg):
        ...
        for e in self.e_layers:
            ...
            e_l = self.e_p_length
            p = tensor_prompt(self.e_pool_size, e_l, policy_cfg.embed_size)
...
class BCHierarchicalTransformerCPPromptMTPolicy(BasePolicy):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def __init__(self, cfg, shape_meta):
        ...
        self.n_tasks = policy_cfg.n_tasks
        # self.cp_c_selection_cof = policy_cfg.cp_c_selection_cof
        ...
        # self.CP_C = nn.Parameter(torch.zeros([self.n_tasks, self.R, self.R, self.R],
        #                                      dtype=torch.float), requires_grad=True)
        self.CP_C = nn.Parameter(torch.zeros([self.R, self.R, self.R], dtype=torch.float), requires_grad=True)
        # self.CP_CK = tensor_prompt(self.n_tasks, 512)  # 因为任务目标文本编码后的embedding维度为512
        ...
        self.temporal_transformer_skill = CP_TransformerDecoder_Skill_Prompt(
    ...
    def skill_inference_decode(self, x, CP_C):
        ...
        x, prompt_loss = self.temporal_transformer_skill(x, self.prompt, CP_U=self.CP_U, CP_V=self.CP_V, CP_C=CP_C)
        x = x.reshape(*sh)  # (B, T:10, num_modalities:5, E:384)
        # return x[:, :, 0], prompt_loss  # (B, T, E)
        return x, prompt_loss  # (B, T, E)

    def action_execute_decode(self, x, CP_C):
        ...
        x = x + pos_emb.unsqueeze(1)  # (B, T, num_modality, E)  改了操作
        sh = x.shape  # (1, 10, 1, 64)
        self.temporal_transformer_action.compute_mask(x.shape)

        x = TensorUtils.join_dimensions(x, 1, 2)  # (B, T*num_modality, E)
        x = self.temporal_transformer_action(x, CP_U=self.CP_U, CP_V=self.CP_V, CP_C=CP_C)
        x = x.reshape(*sh)
        return x[:, :, 0]  # (B, T, E)
    ...
    def cp_c_selection(self, data):
        # if self.training:
        #     if self.prompt.task_count > 0:
        #         CP_C_TMP = torch.cat((self.CP_C[:self.prompt.task_count].detach().clone(),
        #                               self.CP_C[self.prompt.task_count].unsqueeze(0)), dim=0)
        #         CP_CK_TMP = torch.cat((self.CP_CK[:self.prompt.task_count].detach().clone(),
        #                                self.CP_CK[self.prompt.task_count].unsqueeze(0)), dim=0)
        #     else:
        #         CP_C_TMP = self.CP_C[0].unsqueeze(0)
        #         CP_CK_TMP = self.CP_CK[0].unsqueeze(0)
        # else:
        #     if self.prompt.task_count == 0:
        #         CP_C_TMP = self.CP_C[0].unsqueeze(0)
        #         CP_CK_TMP = self.CP_CK[0].unsqueeze(0)
        #     else:
        #         CP_C_TMP = torch.cat((self.CP_C[:self.prompt.task_count],
        #                               self.CP_C[self.prompt.task_count].unsqueeze(0)), dim=0)
        #         CP_CK_TMP = torch.cat((self.CP_CK[:self.prompt.task_count],
        #                                self.CP_CK[self.prompt.task_count].unsqueeze(0)), dim=0)
        # current_task_embedding = data['task_emb'][0]  # (512)
        # print("CP_CK_TMP", CP_CK_TMP.shape)
        # n_CP_CK_TMP = nn.functional.normalize(CP_CK_TMP, dim=1)  # (current_task_id, 512)
        # n_current_task_embedding = nn.functional.normalize(current_task_embedding, dim=0)
        # cos_similarity = torch.einsum('d,kd->k', n_current_task_embedding, n_CP_CK_TMP)  # 余弦相似度
        # n_cos_similarity = F.softmax(cos_similarity, dim=-1)
        # print("n_cos_similarity", n_cos_similarity)
        # # 选最大的一个
        # CP_C_index = torch.argmax(n_cos_similarity, dim=0)
        # print("CP_C_index", CP_C_index)
        # CP_C = CP_C_TMP[CP_C_index]
        #
        # cp_c_selection_max_loss = - n_cos_similarity[self.prompt.task_count] * self.cp_c_selection_cof
        #
        # # 加权
        # # CP_C = torch.einsum('k,kabc->abc', n_cos_similarity, CP_C_TMP)
        CP_C = self.CP_C
        # cp_c_selection_max_loss = torch.zeros((1,)).to(data['task_emb'][0].device)
        # return CP_C, cp_c_selection_max_loss
        return CP_C

    def forward(self, data):
        # CP_C, cp_c_loss = self.cp_c_selection(data)
        CP_C = self.cp_c_selection(data)
        x = self.spatial_encode(data)  # torch.Size([B:32, T:10, num_modalities:5, E])
        x, prompt_loss = self.skill_inference_decode(x, CP_C)  # (B:32, T:10, E)
        # print("after skill_inference_decode shape: ", x.shape)
        x = self.action_execute_decode(x, CP_C)
        # print("after action_execute_decode shape: ", x.shape)   (B, T, E)
        dist = self.policy_head(x)  # .sample()能够得到动作 [B:32, T:10, ac_dim:7]
        # return dist, prompt_loss, cp_c_loss
        return dist, prompt_loss

    def get_action(self, data):
        self.eval()
        with torch.no_grad():
            # CP_C, _ = self.cp_c_selection(data)
            CP_C = self.cp_c_selection(data)
            ...
            x, _ = self.skill_inference_decode(x, CP_C)
            x = self.action_execute_decode(x, CP_C)
