...
class BC2TransformerPolicy(BasePolicy):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def __init__(self, cfg, shape_meta):
        ...
        self.temporal_transformer_1 = TransformerDecoder(
        ...
        self.temporal_transformer_2 = TransformerDecoder(
            input_size=embed_size,
            num_layers=policy_cfg.transformer_num_layers,
            num_heads=policy_cfg.transformer_num_heads,
            head_output_size=policy_cfg.transformer_head_output_size,
    ...
    def temporal_encode(self, x):
        pos_emb = self.temporal_position_encoding_fn(x)  # (T, E)
        x = x + pos_emb.unsqueeze(1)  # (B, T, num_modality, E)
        sh = x.shape
        self.temporal_transformer_1.compute_mask(x.shape)

        x = TensorUtils.join_dimensions(x, 1, 2)  # (B, T*num_modality, E)
        x = self.temporal_transformer_1(x)  # (B, T*num_modality, E)

        # pos_emb = self.temporal_position_encoding_fn(x)
        # x = x + pos_emb.unsqueeze(1)  # (B, T, num_modality, E)
        # print("after transformer2 position encoding: ", x.shape)
        self.temporal_transformer_2.compute_mask(sh)

        x = self.temporal_transformer_2(x)
        # print("after temporal transformer shape: ", x.shape)
        x = x.reshape(*sh)
    ...
    def get_action(self, data):
        ...
        action = dist.sample().detach().cpu()  # 动作采样
