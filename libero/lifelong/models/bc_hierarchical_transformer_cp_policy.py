...
class BCHierarchicalTransformerCPPolicy(BasePolicy):
    ...
    def skill_inference_decode(self, x):
        ...
        sh = x.shape
        self.temporal_transformer_skill.compute_mask(x.shape)
        x = TensorUtils.join_dimensions(x, 1, 2)  # (B, T*num_modality, E)
        x = self.temporal_transformer_skill(x, CP_U=self.CP_U, CP_V=self.CP_V, CP_C=self.CP_C)
    ...
    def cp_c_selection(self, data):
        if self.training:
            if self.task_count > 0:
                CP_C_TMP = torch.cat((self.CP_C[:self.task_count].detach().clone(),
                                      self.CP_C[self.task_count].unsqueeze(0)), dim=0)
                CP_CK_TMP = torch.cat((self.CP_CK[:self.task_count].detach().clone(),
                                       self.CP_CK[self.task_count].unsqueeze(0)), dim=0)
            else:
            ...
            if self.task_count == 0:
                ...
                CP_C_TMP = torch.cat((self.CP_C[0: self.task_count],
                                      self.CP_C[self.task_count].unsqueeze(0)), dim=0)
                CP_CK_TMP = torch.cat((self.CP_CK[0: self.task_count],
                                       self.CP_CK[self.task_count].unsqueeze(0)), dim=0)
        ...
        # 加权
        # CP_C = torch.einsum('k,kabc->abc', cos_similarity, CP_C_TMP)
