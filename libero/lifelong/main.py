...
def main(hydra_cfg):
    ...
    for i in range(n_manip_tasks):
        ...
        try:
            task_i_dataset, shape_meta = get_dataset(
                ...
                load_previous_action=cfg.data.load_previous_action,
            )
    ...
    task_embs = get_task_embs(cfg, descriptions)  # (n_tasks: 10, 512)
    benchmark.set_task_embs(task_embs)
    print("\n=================== Benchmark Shape_Meta Information  ===================")
    for k in shape_meta.keys():
        print(k, shape_meta[k])
    print("=======================================================================\n")
    ...
    # define lifelong algorithm  同时也定义了使用什么policy，各种编码器等
    ...
    if cfg.pretrain_model_path != "":  # load a pretrained model if there is any
        try:
            ...
            print("[info] Pretrained model successfully loaded!")
        except:
            print(
    ...
    if cfg.lifelong.algo == "Multitask":
        ...
        if cfg.eval.eval:
            total_losses, prompt_losses, cp_c_losses = evaluate_loss(cfg, algo, benchmark, datasets)
            L = total_losses - prompt_losses - cp_c_losses
            # total_losses, prompt_losses = evaluate_loss(cfg, algo, benchmark, datasets)
            # L = total_losses - prompt_losses
            S = evaluate_success(
                cfg=cfg,
                algo=algo,
                benchmark=benchmark,
            ...
            print(("[All task BC loss ] " + " %4.4f |" * n_tasks) % tuple(L))
            print(("[All task succ.] " + " %4.4f |" * n_tasks) % tuple(S))
        ...
        for i in range(n_tasks):
            ...
            result_summary["S_fwd"][i] = s_fwd  # 50轮训练过程中每个任务的平均成功率
            result_summary["L_fwd"][i] = l_fwd
            t1 = time.time()

            # evaluate on all seen tasks at the end of learning each task
            if cfg.eval.eval:
                total_losses, prompt_losses, cp_c_losses = evaluate_loss(cfg, algo, benchmark, datasets[: i + 1])
                L = total_losses - prompt_losses - cp_c_losses
                # total_losses, prompt_losses = evaluate_loss(cfg, algo, benchmark, datasets[: i + 1])
                # L = total_losses - prompt_losses
                t2 = time.time()
                S = evaluate_success(
                    cfg=cfg,
                ...
                print(("[Task %2d BC loss ] " + " %4.4f |" * (i + 1)) % (i, *L))
                print(("[Task %2d succ.] " + " %4.4f |" * (i + 1)) % (i, *S))
                torch.save(
...
if __name__ == "__main__":
    ...
    multiprocessing.set_start_method("spawn", force=True)  # 否则HDF5文件无法多线程读取
    main()
